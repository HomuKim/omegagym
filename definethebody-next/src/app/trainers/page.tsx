  ...
  const renderCard = (trainer: Trainer, isGrid: boolean = false) => {
    return (
      <motion.div 
        key={trainer.id}
        whileHover={{ y: -3 }}
        className={`relative overflow-hidden rounded-[15px] shadow-sm cursor-pointer transition-all duration-300 leading-none bg-gradient-to-br from-[#090404] to-[#968a8a] group
      ...
      >
        ...
        />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-[2.5rem] max-sm:text-[28px] font-bold text-center w-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-pre-line leading-[1.5]">
          {trainer.name}
        </div>
      </motion.div>
    );
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <motion.section 
        ...
        className="relative flex flex-col items-center justify-center bg-black/60 text-white text-center py-1 px-5 mt-[30px] mb-[30px] mx-auto max-w-[1160px] min-h-[280px] max-sm:min-h-[175px] max-sm:w-[350px] max-sm:p-2.5 rounded-[20px] overflow-hidden"
      >
        <div className="absolute inset-0 bg-[url('/images/logo.png')] bg-[length:50%] max-sm:bg-[length:100%] bg-center bg-no-repeat opacity-30 z-[-1]" />
        <h1 className="text-5xl max-sm:text-[35px] mb-5 font-bold drop-shadow-[2px_2px_4px_rgba(0,0,0,0.5)]">
          함께하는 피트니스 여정
        </h1>
        <p className="text-xl max-sm:text-[17px] max-w-[600px] mx-auto mb-[30px] drop-shadow-[1px_1px_2px_rgba(0,0,0,0.5)]">
          디파인더바디의 열정적인 스태프들을 만나보세요
        </p>
      </motion.section>

      {/* Admin Section */}
      <section className="my-10 text-center flex flex-col items-center w-full">
        <h2 className="text-2xl font-bold mb-5">운영진</h2>
      ...
      <section className="my-10 mb-20 flex flex-col items-center w-full">
        <h2 className="text-2xl font-bold mb-5">트레이너</h2>
        ...
        {selectedTrainer && (
          ...
          >
            ...
            <motion.div 
              ...
              className="relative flex flex-row max-sm:flex-col bg-white w-[80%] h-[80%] max-sm:w-[calc(100%-20px)] max-sm:h-auto max-sm:max-h-[85vh] rounded-[10px] overflow-hidden max-sm:p-2.5"
            >
              <button 
                onClick={closeModal} 
              ...
              <div className="p-[30px] px-[80px] max-sm:p-2.5 flex flex-col justify-center bg-gray-50 border-r max-sm:border-r-0 max-sm:border-b border-gray-200">
                <ul className="list-none p-0 mx-0 my-auto flex flex-col max-sm:flex-row max-sm:justify-around max-sm:w-full gap-4">
                  {[
                    ...
                    <li 
                      ...
                      className={`text-[40px] max-sm:text-sm font-bold text-center py-[15px] px-[10px] cursor-pointer transition-all duration-300
                        ${activeTab === tab.id 
                          ? "text-primary-red drop-shadow-md max-sm:border-b-2 max-sm:border-primary-red" 
                  ...
                  {selectedTrainer.instagram && (
                    <li className="text-[40px] max-sm:text-sm font-bold text-center py-[15px] px-[10px] text-gray-400 hover:text-gray-600 transition-colors">
              ...
              {/* Content Area */}
              <div className="relative flex-1 p-5 overflow-y-auto w-full h-full bg-white">
                  ...
                  <motion.div initial={{opacity:0}} animate={{opacity:1}} className="flex flex-col items-center gap-5 pb-10">
                    {validReviews.length > 0 ? validReviews.map((src, i) => (
                      <img 
                        ...
                        src={src} 
                        alt={`Review ${i+1}`} 
                        className="w-[75%] max-sm:w-full h-auto rounded-[15px] shadow-sm" 
                      />
                    )) : (
                      <div className="text-gray-400 mt-20">등록된 후기가 없습니다.</div>
                    )}
                  </motion.div>
                )}
