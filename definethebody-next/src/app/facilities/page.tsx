  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <motion.section 
        ...
        className="relative flex flex-col items-center justify-center bg-black/60 text-white text-center py-1 px-5 mt-[30px] mb-[30px] mx-auto max-w-[1160px] min-h-[280px] max-sm:min-h-[175px] max-sm:max-h-[175px] max-sm:w-[350px] max-sm:p-2.5 rounded-[20px] overflow-hidden leading-[1.6]"
      >
        <div className="absolute inset-0 bg-[url('/images/logo.png')] bg-[length:50%] max-sm:bg-contain bg-center bg-no-repeat opacity-30 z-[-1]" />
        <h1 className="text-5xl max-sm:text-[33px] mb-5 font-bold drop-shadow-[2px_2px_4px_rgba(0,0,0,0.5)]">건강한 삶을 위한 완벽한 환경</h1>
        <p className="text-xl max-sm:text-[16px] max-w-[600px] mx-auto mb-[30px] drop-shadow-[1px_1px_2px_rgba(0,0,0,0.5)]">최고의 운동 환경과 다양한 시설을 갖춘<br/>디파인더바디 피트니스에 오신 것을 환영합니다.</p>
      </motion.section>
            ...
            {facilityTabs.map((tab) => (
              <li 
                ...
                className={`text-[25px] max-lg:text-[18px] max-sm:text-[14px] font-bold px-0 py-4 max-lg:px-4 max-lg:py-2.5 cursor-pointer uppercase transition-all duration-300 relative
                  ${activeTab === tab.id 
                    ? "text-primary-red" 
                    : "text-gray-400 hover:text-black hover:pl-[10px] max-lg:hover:pl-4"}
                  ${activeTab === tab.id ? "after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-primary-red max-lg:after:h-[1px]" : ""}
                `}
              >
                {tab.label}
              </li>
          ...
          <div className="flex flex-col gap-10">
            {images.map((src, i) => (
              <motion.div 
                ...
                className="w-full rounded-[10px] overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer"
              >
